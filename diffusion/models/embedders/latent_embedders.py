...
from diffusion.models.utils.conv_blocks import DownBlock, UpBlock, BasicBlock, BasicResBlock, UnetResBlock, UnetBasicBlock
from diffusion.loss.gan_losses import hinge_d_loss
from diffusion.loss.perceivers import LPIPS
from diffusion.models.model_base import BasicModel, VeryBasicModel
...
class VQGAN(VeryBasicModel):
    def __init__(
        ...
        self.vqvae = VQVAE(data_name, in_channels, out_channels, spatial_dims, emb_channels, num_embeddings, hid_chs, kernel_sizes,
...
class VAE(BasicModel):
    ...
    def _step(self, batch: dict, batch_idx: int, state: str, step: int, optimizer_idx:int):
        ...
        # ----------------- Save Image ------------------------------
        #print(self.global_step, self.current_epoch, self.sample_every_n_steps)
        if self.global_step != 0 and self.global_step % self.sample_every_n_steps == 0:
            log_step = self.global_step // self.sample_every_n_steps
            path_out = Path(self.logger.log_dir)/'images'
            path_out.mkdir(parents=True, exist_ok=True)
            ...
            images = torch.cat([depth2batch(img)[:16] for img in (x, pred)]) 
            save_image(images, path_out/f'sample_{log_step}.png', nrow=x.shape[0], normalize=True)
            #save_image(images, path_out/f'sample_{log_step}_S.png', nrow=x.shape[0])
    
        return loss



class Latent_Pipeline(BasicModel):
    def __init__(
        self,
        ...
        embedding_loss_weight=1e-6,
        vae_loss_weight=1e-3,
        clip_loss_weight=1e-1,
        logit_scale=1e-1,
        ...
        **kwargs
        ...
        self.latent_embedder = self.instantiate_first_stage(latent_embedder, latent_embedder_checkpoint, requires_grad=False)
        ...
        self.latent_embedder_condition = self.cond_latent_embedder[self.img_condition_name]
   

    @torch.no_grad()
    def instantiate_first_stage(self, latent_embedder=None, latent_embedder_checkpoint=None, requires_grad=True):
    ...
    def forward(self, batch):
        ...
        condition = batch[self.img_condition_name]

        pred, pred_vertical, emb_loss = self.latent_embedder_condition(condition)

        # ------------------------- rec Loss ---------------------------
        target = condition
        loss = self.latent_embedder_condition.rec_loss(pred, pred_vertical, target)
        loss += emb_loss*self.embedding_loss_weight
        
        vae_loss = loss
        # ----------------------- clip Loss ---------------------------
        z = self.latent_embedder.encode(x_0)
        c = self.latent_embedder_condition.encode(condition)

        z = z.sigmoid()
        c = c.sigmoid()

        z = z.flatten(1)  # b,c,h,w ---> b,*
        c = c.flatten(1)

        logits_per_z = self.logit_scale * z @ c.T
        logits_per_c = self.logit_scale * c @ z.T

        labels = torch.arange(c.shape[0], device=self.device, dtype=torch.long)

        clip_loss = (
            F.cross_entropy(logits_per_z, labels) +
            F.cross_entropy(logits_per_c, labels)
        ) / 2

        return vae_loss, clip_loss, pred, target
    ...
    def _step(self, batch: dict, batch_idx: int, state: str, step: int, optimizer_idx:int):
        # ------------------------- Run Model ---------------------------
        x = batch['source']
        vae_loss, clip_loss, pred, target  = self(batch)

        loss = self.vae_loss_weight*vae_loss + self.clip_loss_weight*clip_loss

        # ------------------------- Compute Loss ---------------------------
         
        # --------------------- Compute Metrics  -------------------------------
        with torch.no_grad():
            logging_dict = {'loss':loss, 'vae_loss': self.vae_loss_weight*vae_loss, 'clip_loss':self.clip_loss_weight*clip_loss}
            logging_dict['L2'] = torch.nn.functional.mse_loss(pred, target)
            logging_dict['L1'] = torch.nn.functional.l1_loss(pred, target)
            logging_dict['ssim'] = ssim((pred+1)/2, (target.type(pred.dtype)+1)/2, data_range=1)
            # print(logging_dict)
            # print(logging_dict['ssim'])
            # logging_dict['logvar'] = self.logvar
        ...
        if self.global_step != 0 and self.global_step % self.sample_every_n_steps == 0:
            ...
            def depth2batch(image):
                return (image if image.ndim<5 else torch.swapaxes(image[0], 0, 1))
            images = torch.cat([depth2batch(img)[:16] for img in (x, pred)]) 
            save_image(images, path_out/f'sample_{log_step}_N.png', nrow=x.shape[0], normalize=True)
            save_image(images, path_out/f'sample_{log_step}.png', nrow=x.shape[0], normalize=False)
            #save_image(images, path_out/f'sample_{log_step}_S.png', nrow=x.shape[0])
    ...
    def on_save_checkpoint(self, checkpoint):
        path_out = Path(self.logger.log_dir)
        torch.save(self.cond_latent_embedder[self.img_condition_name].state_dict(), path_out/f'clip_vae_{self.img_condition_name}_epoch={self.current_epoch}_step={self.global_step}.pt')



class CLIP_VAE(BasicModel):
    def __init__(
        self,
        latent_embedder=None,
        latent_embedder_checkpoint='',
    ...
    def _step(self, batch: dict, batch_idx: int, state: str, step: int, optimizer_idx:int):
        ...
        condition = batch[self.data_name]

        pred, pred_vertical, emb_loss = self(condition)
        ...
        vae_loss = loss
        # ----------------------- clip Loss ---------------------------
        z = self.latent_embedder.encode(x)
        ...
        if self.global_step != 0 and self.global_step % self.sample_every_n_steps == 0:
            ...
            def depth2batch(image):
                return (image if image.ndim<5 else torch.swapaxes(image[0], 0, 1))
            # images = torch.cat([img for img in (x, pred)]) 
            x, pred = x*2-1, pred*2-1
            x, pred = x.clamp(0,1), pred.clamp(0,1)
            images = torch.concat([x, pred], dim=0)
            save_image(images, path_out/f'sample_{log_step}_N.png', nrow=x.shape[0], normalize=True)
            save_image(images, path_out/f'sample_{log_step}.png', nrow=x.shape[0], normalize=False)
            #save_image(images, path_out/f'sample_{log_step}_S.png', nrow=x.shape[0])
